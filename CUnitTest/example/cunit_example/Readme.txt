...
分别返回最大值和最小值，并在头文件中进行接口函数的声明。同时声明了内部函数，函
数名为inter_maxi()，使用static修饰，用以演示如何测试模块内部函数。这里使用的方
法是根据宏UT是否定义来决定是否使用static进行修饰，从而决定其能够被外部模块所调
用。同时在对应的头文件中进行相同的处理。对应的宏UT则是在单元测试的Makefile中进
行定义的。希望读者能给出更优雅的方法。
...
这里Makefile在编译项目代码的目标文件时，使用的CFLAGS中添加了-DUT参数，表示定义
了宏UT。这是为了能够访问项目代码中的模块内部函数。同时项目代码中，考虑到严格的
模块隔离，并没有使用该宏。这样的话，会造成两者生成的目标文件不一致的情况，结果
就是app目标和test目标不能连续地执行，必须要首先执行clean目标，以清除掉冲突的目标
文件。这个只是权益之策，希望能给出

此外，因为测试程序依赖项目代码中的模块，所以如果项目代码没有编译，而要求首先编
译测试代码时，make会根据依赖规则首先生成$(EXE_OBJ)指定的目标文件，满足测试程序
需要的条件后再生成测试程序。 这样能够确保所有被测试的代码在$(EXEC)被运行之前都
被更新和编译。
...
的函数实现和模块架构并未做说明。另外，对于进行单元测试对项目代码的干扰和污染也
没有进行讨论。 这些还需要读者阅读相关资料，并在实际项目实践中不断加深对CUnit和
单元测试的认识和理解，并对该文档进行有效地修正和补充。
